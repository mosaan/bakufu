...
class ProviderExtraParams(TypedDict, total=False):
    """Type definition for provider-specific extra parameters"""
    ...
    system: str  # System message for some providers
    tools: list[dict[str, Any]]  # Tool definitions
    tool_choice: str | dict[str, Any]  # Tool selection strategy


class UsageStats(TypedDict, total=False):
    """Type definition for AI usage statistics from LiteLLM"""
...
class CompletionKwargs(TypedDict, total=False):
    """Type definition for completion method kwargs"""
...
class AIProviderConfig(BaseModel):
    ...
    # Provider-specific settings (using dict[str, Any] for flexibility with external libraries)
...
class BaseAIProvider(ABC):
    ...
    @abstractmethod
    async def complete(self, prompt: str, **kwargs: CompletionKwargs) -> AIResponse:
    ...
    def _get_temperature(self, kwargs: CompletionKwargs) -> float:
        """Get temperature parameter with fallback to config"""
        temp = kwargs.get("temperature", self.config.temperature)
        return float(temp)

    def _get_max_tokens(self, kwargs: CompletionKwargs) -> int | None:
...
class AIProvider(BaseAIProvider):
    ...
    async def stream_complete(
        self, prompt: str, **kwargs: CompletionKwargs
...
class MCPSamplingProvider(BaseAIProvider):
    ...
    async def complete(self, prompt: str, **kwargs: Any) -> AIResponse:
        ...
        try:
            ...
            temperature = self._get_temperature(kwargs)  # type: ignore[arg-type]
            max_tokens = self._get_max_tokens(kwargs)  # type: ignore[arg-type]
